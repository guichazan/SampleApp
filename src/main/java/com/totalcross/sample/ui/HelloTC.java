...
public class HelloTC extends MainWindow {
    ...
    // fmH is the font size. We will set the components to have twice the size of font.
    ...
    public void initUI() {
	try {
	    // add the logo at top. Copy your logo to img folder and try to change it.
	    ...
	    add(new Label("Name: "), LEFT + FLAT_EDGE_MARGIN, AFTER + 100, FILL - FLAT_EDGE_MARGIN, COMPONENT_H);
	    ...
	    add(new Label("Phone Number: "), LEFT + FLAT_EDGE_MARGIN, AFTER, SAME, SAME);
	    ...
	    add(btInsert = new Button("Insert"), BEFORE, SAME, PARENTSIZE + 40, COMPONENT_H, sp);
	    add(btClear = new Button("Clear"), AFTER, SAME, PARENTSIZE + 40, COMPONENT_H, sp);
	    ...
	    btClear.addPressListener((event) -> {
		clear();
	     ...
	     * Settings.isIOS() returns true when running on iOS..
	     */
